...
%PYTHON_DIR%\python.exe %PCBASIC_PATH%\run.py --interface=sld2 --mount=C:%MOUNT_C%,D:%MOUNT_D% --run=%PROGRAM% --quit=False -f=10 --shell="python %BRWFUNCT_DIR%\Brw_functions.py" --extension=Brw_extensions_simple1 --debug=False --logfile=%LOG_DIR%\pcbasic_brewer_log.txt
