...
set PCBASIC_PATH=C:\PCBasic_Brewer_Repo\pcbasic
...
%PCBASIC_PATH%\ansipipe-launcher.exe %PYTHON_DIR%\python.exe %PCBASIC_PATH%\run.py --mount=C:%MOUNT_C%,D:%MOUNT_D% --interface=ansi --run=%PROGRAM% --quit=False -f=10 --logfile=%LOG_DIR%\pcbasic_log.txt
