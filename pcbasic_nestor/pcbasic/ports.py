...
def prepare():
    ...
    # print 'Initializating the GWBASIC Com Port 1'
    logging.debug('Initializating the GWBASIC Com Port 1, with serial_in_size=%s', serial_in_size)
    state.io_state.devices['COM1:'] = COMDevice(config.get('com1'), max_reclen, serial_in_size)
    # print 'Initializating the GWBASIC Com Port 2'
...
class COMDevice(devices.Device):
    ...
    def __init__(self, arg, max_reclen, serial_in_size):
        ...
        logging.debug("Initialize the COM port: %s", val)
        ...
        try:
            if not addr and not val:
                ...
                #print 'Could not attach %s to COM device', arg
                logging.warning('Could not attach %s to COM device', arg)
        except (ValueError, EnvironmentError) as e:
            #print 'Could not attach %s to COM device', arg, e
            logging.warning('Could not attach %s to COM device: %s', arg, e)
            self.stream = None
        except AttributeError as e:
            #print 'Serial module not available. Could not attach %s to COM device:', arg
