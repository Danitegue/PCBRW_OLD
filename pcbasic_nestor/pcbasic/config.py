...
arguments = {
    ...
    'use-serial-brewer': { 'type' : 'bool', 'default': False },
...
def get_arguments(argv):
    ...
    for arg in argv:
        #logger.debug(u'Reading arg "=%s"', arg)
        if arg not in pycharm_args and 'pcbasic.py' not in arg and "PyCharm" not in arg:
            ...
            if key:
                if key[0:2] == u'--':
                    ...
                    try:
                        testf = int(arg)
                        argisint = True
                        logger.warning(u'Ignored pycharm int args "=%s"', arg)
                    except:
                        argisint = False
            ...
            logger.warning(u'Ignored pycharm args "=%s"', arg)
    return args
