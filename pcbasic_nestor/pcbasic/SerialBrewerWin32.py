...
class Serial(SerialBase):
    ...
    #BAUDRATES = (1200,9600, 300, 50, 75, 110, 134, 150, 200, 600, 1800, 2400, 4800,
    ...
    def open(self):
        ...
        """
        if self.verbose:
            sys.stdout.write("Using SerialBrewer Nestor module for oppening communications\n")
        logging.info("Using SerialBrewer Nestor module for oppening communications")
        ...
        try:
            ...
            pass
        self.hComPort = win32.CreateFile(
                port,
    ...
    def _reconfigurePort(self):
        ...
        # #
        # # if self._writeTiemout is None:
        # #     pass
        # # elif self._writeTiemout == 0:
        # #     timeouts.WriteTotalTimeoutConstant = win32.MAXDWORD
        # # else:
        # #     timeouts.WriteTotalTimeoutConstant = max(int(self._writeTiemout * 1000), 1)
        # # win32.SetCommTimeouts(self.hComPort, ctypes.byref(timeouts))
        #
        # win32.SetCommMask(self.hComPort, win32.EV_ERR)
        #
    ...
    def read(self, size=1):
        ...
        if size > 0:
            ...
            if not win32.ClearCommError(self.hComPort, ctypes.byref(flags), ctypes.byref(comstat)):
                raise SerialException('call to ClearCommError failed')
            if self.timeout == 0: #Case of no timeout
                #logging.debug("tiemout for read = 0")
                ...
                if n > 0:
                    ...
                    if not err1:
                        ...
                        if err2 != 0:
                            ...
                            read = buf.raw[:rc.value]
                            logging.debug("SerialBrewerWin32.py read function1 has read: %s",str(read).replace('\r', '\\r').replace('\n', '\\n').replace('\x00', '\\x00'))
                        else:
                    ...
                    #logging.warning("Not enough bytes in the buffer, n=%s, comstat.cbInQue=%s, size=%s",
                    #                str(n), str(comstat.cbInQue), str(size))
                    read = bytes()
            else:
                # case self.timeout != 0
                #logging.debug("tiemout for read != 0")
                ...
                if not err1:
                    ...
                    if err2 != 0:
                        # rc.value is zero, so the read is always empty.
                        read = buf.raw[:rc.value]
                        logging.debug("SerialBrewerWin32.py read function2 has read: %s",str(read).replace('\r', '\\r').replace('\n', '\\n').replace('\x00', '\\x00'))
                    else:
                        logging.warning("Error in GetOverlappedResult = %s", str(err2))
                        #print "Error in GetOverlappedResult"
                        read = bytes()
            ...
            read = bytes()
        #if len(read) != 0:
        #    logging.warning("Read the following: %s, %s", str(bytes(read)), str(len(read)))
        return bytes(read)
        ...
        if data:
            ...
            if not err:
                ...
                if errcode != win32.ERROR_IO_PENDING:
                    ...
                    #else:
                    #    logging.warning('Got GetOverlappedResult write finished correctly')
                # if n.value != len(data):
            ...
            if self._writeTiemout != 0:  # if blocking (None) or w/ write timeout (>0)
                # Wait for the write to complete.
                #~ win32.WaitForSingleObject(self._overlappedWrite.hEvent, win32.INFINITE)
